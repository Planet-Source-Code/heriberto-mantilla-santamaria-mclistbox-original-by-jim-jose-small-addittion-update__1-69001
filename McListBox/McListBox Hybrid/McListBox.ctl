...
Private m_IconFolderOpen As StdPicture
Private m_IconFolderClose As StdPicture
Private m_IconCDROM As StdPicture
Private m_IconRAMDisk As StdPicture
Private m_IconDriver As StdPicture
Private m_IconRemovable As StdPicture
Private m_CustomIcons As Boolean
Private m_Drivers() As String
...
Private Const m_def_BorderStyle = BDR_SUNKEN
...
Private Const m_def_AutoHideScrollBars = False
Private Const m_def_Mode = 0
Private Const m_def_Path = vbNullString
Private Const m_def_IconExtractSize = 0
Private Const m_def_AutoRefresh = True
Private Const m_def_Filter = "*.*"
Private Const m_def_ShowSystemFiles = False
Private Const m_def_ShowHiddenFiles = False
...
Public Property Let ListIndex(ByVal vNewValue As Long)
    ...
    If vNewValue < 0 Or vNewValue > m_Items.Count - 1 Then
        m_SelItem = -1
        Exit Property
    End If
...
Private Sub UserControl_InitProperties()
    ...
    Set m_IconFolderOpen = Nothing
    Set m_IconFolderClose = Nothing
    Set m_IconRemovable = Nothing
    Set m_IconRAMDisk = Nothing
    Set m_IconDriver = Nothing
    m_CustomIcons = False
...
On Error GoTo handle
    ...
    If m_GridLines Then
        UserControl.ForeColor = m_GridColor
        Line (0, mTop + m_RowHeight)-(ScaleWidth, mTop + m_RowHeight)
        UserControl.ForeColor = m_ForeColor
    End If
...
handle:
    ...
    If (m_CustomIcons = True) Then
        If (IsDriver(sFileName) = True) Then
            ...
            Case 2
                m_FileIcons.Add m_IconRemovable, sKey
            Case 3
                m_FileIcons.Add m_IconDriver, sKey
            Case Is = 4
                m_FileIcons.Add m_IconRemovable, sKey
            Case Is = 5
                m_FileIcons.Add m_IconCDROM, sKey
            Case Is = 6
                m_FileIcons.Add m_IconRAMDisk, sKey
            Case Else
        ...
        ElseIf (IsFolder(sFileName & "\") <> "") Then
            ...
            If (IsBol = True) Then
                m_FileIcons.Add m_IconFolderOpen, sKey
                Set GetIcon = m_FileIcons(sKey)
            Else
                m_FileIcons.Add m_IconFolderClose, sKey
                Set GetIcon = m_FileIcons(sKey)
            End If
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    Call PropBag.WriteProperty("IconFolderOpen", m_IconFolderOpen, Nothing)
    Call PropBag.WriteProperty("IconFolderClose", m_IconFolderClose, Nothing)
    Call PropBag.WriteProperty("IconRemovable", m_IconRemovable, Nothing)
    Call PropBag.WriteProperty("IconCDROM", m_IconCDROM, Nothing)
    Call PropBag.WriteProperty("IconDriver", m_IconDriver, Nothing)
    Call PropBag.WriteProperty("IconRAMDisk", m_IconRAMDisk, Nothing)
    Call PropBag.WriteProperty("CustomIcons", m_CustomIcons, False)
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    Set m_IconRemovable = PropBag.ReadProperty("IconRemovable", Nothing)
    Set m_IconRAMDisk = PropBag.ReadProperty("IconRAMDisk", Nothing)
    Set m_IconCDROM = PropBag.ReadProperty("IconCDROM", Nothing)
    Set m_IconDriver = PropBag.ReadProperty("IconDriver", Nothing)
    m_CustomIcons = PropBag.ReadProperty("CustomIcons", False)
...
Public Property Get IconFolderOpen() As StdPicture
    
    Set IconFolderOpen = m_IconFolderOpen
    
End Property

Public Property Set IconFolderOpen(ByVal hIcon As StdPicture)
    
    Set m_IconFolderOpen = hIcon
    PropertyChanged "IconFolderOpen"
    If (m_CustomIcons = True) Then
        ReDrawList
    End If
...
Public Property Get IconFolderClose() As StdPicture
    
    Set IconFolderClose = m_IconFolderClose
    
End Property

Public Property Set IconFolderClose(ByVal hIcon As StdPicture)
    
    Set m_IconFolderClose = hIcon
    PropertyChanged "IconFolderClose"
    If (m_CustomIcons = True) Then
        ReDrawList
    End If
...
Public Property Get IconDriver() As StdPicture
    
    Set IconDriver = m_IconDriver
    
End Property

Public Property Set IconRemovable(ByVal hIcon As StdPicture)
    
    Set m_IconRemovable = hIcon
    PropertyChanged "IconRemovable"
    If (m_CustomIcons = True) Then
        ReDrawList
    End If
...
Public Property Get IconRemovable() As StdPicture
    
    Set IconRemovable = m_IconRemovable
    
End Property

Public Property Set IconCDROM(ByVal hIcon As StdPicture)
    
    Set m_IconCDROM = hIcon
    PropertyChanged "IconCDROM"
    If (m_CustomIcons = True) Then
        ReDrawList
    End If
...
Public Property Get IconCDROM() As StdPicture
    
    Set IconCDROM = m_IconCDROM
    
End Property

Public Property Set IconRAMDisk(ByVal hIcon As StdPicture)
    
    Set m_IconRAMDisk = hIcon
    PropertyChanged "IconRAMDisk"
    If (m_CustomIcons = True) Then
        ReDrawList
    End If
...
Public Property Get IconRAMDisk() As StdPicture
    
    Set IconRAMDisk = m_IconRAMDisk
    
End Property

Public Property Set IconDriver(ByVal hIcon As StdPicture)
    
    Set m_IconDriver = hIcon
    PropertyChanged "IconDriver"
    If (m_CustomIcons = True) Then
        ReDrawList
    End If
...
Public Property Get SetCustomIcons() As Boolean
    
    SetCustomIcons = m_CustomIcons
    
End Property

Public Property Let SetCustomIcons(ByVal bIcons As Boolean)
    
    m_CustomIcons = bIcons
    PropertyChanged "SetCustomIcons"
    ReDrawList
...
Private Function IsDriver(ByVal sDriver As String) As Boolean
  ...
  Dim mSave As String
    ...
    For keer = 0 To UBound(m_Drivers)
        mSave = m_Drivers(keer)
        If (mSave = sDriver) Then
            IsDriver = True
            Exit For
        End If
...
Private Sub SaveDrivers()
  ...
  Dim l As Integer
    ...
    ' Get all the drives
    GetLogicalDriveStrings 255, strSave
    ' Extract the drives from the buffer and print them on the form
    For keer = 1 To 100
        ...
        mSave = Mid$(strSave, 1, InStr(1, strSave, Chr$(0)))
        strSave = Right$(strSave, Len(strSave) - InStr(1, strSave, Chr$(0)))
        If (mSave <> Chr$(0)) Then
            ReDim Preserve m_Drivers(l)
            m_Drivers(l) = mSave
            l = l + 1
        End If
